...
namespace www.Areas.Admin.Controllers
{
    ...
    public class GalleryController : Controller
    {
        ...
        public ActionResult Index(string _searchKey, int? _parentId, DateTime? _fromDate, DateTime? _toDate, int? _pageIndex)
        {
            ...
            result = _services.GetAll(_searchKey, _fromDate, _toDate, _parentId, "Gallery", _languageId, false, _pageIndex, 20, _userName, null);
            ...
            IEnumerable<DropdownModel> category = _services.Dropdownlist(0, null, "ChuyenMucGallery", _languageId);
        ...
        public ActionResult Detail(Content entity)
        {
            ...
            IEnumerable<DropdownModel> category = _services.Dropdownlist(0, (int)entity.contentId, "ChuyenMucGallery", entity.languageId);
        ...
        public ActionResult ChuyenMuc(string _searchKey, int? _parentId, int? _pageIndex)
        {
            ContentView result;
            int _languageId = 1;
            result = _services.GetAllAdmin(_searchKey, null, null, _parentId, "ChuyenMucGallery", _languageId, false, _pageIndex, 20);
            int totalPage = result?.Total ?? 0;
            ViewBag.TotalPage = totalPage;
            ViewBag.PageIndex = _pageIndex ?? 1;
            ViewBag.SearchKey = string.IsNullOrWhiteSpace(_searchKey) ? string.Empty : _searchKey;
            IEnumerable<DropdownModel> category = _services.Dropdownlist(_parentId.GetValueOrDefault(), null, "ChuyenMucGallery", 1);
            ViewBag._parentId = category.Select(x => new SelectListItem { Text = x.Text, Value = x.Value.ToString() });
            return View(result.ViewContents);
        }
